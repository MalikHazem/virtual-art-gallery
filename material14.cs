...
public class material14 : MonoBehaviour
{
    public string url;
    public string url2;
    string url3;

    public ApiPainting14 p;
    ...
    void Start()
    {
        ...
        btn.onClick.AddListener(delegate { onclick(url, url2); });
    ...
    void onclick(string u1, string u2)
    {
        StopCoroutine(GetText(url));
        url3 = url;
        url = url2 + inputsearch.text;
        p = GetImages.GetApi14(url);
        if (p == null)
        {
